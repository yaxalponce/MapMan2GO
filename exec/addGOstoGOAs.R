...
source(file.path(input.args[[1]], "R", "funks.R"))
...
#' Union of protein GOAs
  a <- paste("mm.2.go$`", paste(mm.2.go.df$MapManBin[i], "`$genes.goa", sep=""), sep="")
...
#' Compare n.GO before and after extension of GOs
mm.2.go.df$n.GO[ which(mm.2.go.df$MapManBin.GO != mm.2.go.df.copy$MapManBin.GO) ]
mm.2.go.df.copy$n.GO[ which(mm.2.go.df$MapManBin.GO != mm.2.go.df.copy$MapManBin.GO) ]

#' Identify GOAs duplicated
coords.duplicated.GOAs <- which(duplicated(mm.2.go.df.copy$MapManBin.GO) | duplicated(mm.2.go.df.copy$MapManBin.GO, fromLast=TRUE) )
duplicated.GOAs <- mm.2.go.df.copy$MapManBin.GO[coords.duplicated.GOAs]
duplicated.bins <- mm.2.go.df.copy$MapManBin[coords.duplicated.GOAs]
...
plot(mm.2.go.df.copy$n.genes, mm.2.go.df.copy$n.GO, xlab = "Number of genes per MapMan-Bin", 
...
plotDistAsHistAndBox(mm.2.go.df.copy$n.GO, "Number of GO Terms per MapMan-Bin GOA")
dev.off()
...
x <- as.numeric(table(mm.2.go.df.copy[which(mm.2.go.df.copy$MapManBin.GO != ""), "MapManBin.GO"]))
...
##' Just for the presentation, delete
which(mm.2.go.df.copy$MapManBin.GO == mm.2.go.df.copy$MapManBin.GO[22])
